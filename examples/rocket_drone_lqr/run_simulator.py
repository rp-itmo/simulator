...
from examples.rocket_drone_lqr.communication.zmq_protocol import ZmqClient
...
def rocket_shape(x: float, z: float, theta: float) -> tuple[list[float], list[float]]:
    body = [(0.0, 0.45), (-0.18, -0.25), (0.18, -0.25), (0.0, 0.45)]
    xs, zs = [], []
...
def main() -> None:
    ...
    for step in range(steps):
        ...
        response_data = client.request(request.to_dict())
    ...
    ax.set_title("2D Rocket Drone LQR trajectory tracking via ZeroMQ")
    ax.set_xlabel("x")
    ax.set_ylabel("z")
